...
async function run() {
  try {
    ...
    const npmrc_options = core.getInput('npmrc-options', { required: false }).trim()
    const ignore = new Set()
    const ignoreList = core.getInput('ignore', { required: false }).trim().split(',')

    function setToken() {
      if (process.env.REGISTRY_TOKEN) {
        console.log('token: '+ process.env.REGISTRY_TOKEN.trim())
        return process.env.REGISTRY_TOKEN.trim()
      } else if (process.env.REGISTRY_CREDENTIALS) {
        console.log('token: '+ process.env.REGISTRY_CREDENTIALS.trim())
        return Buffer.from(process.env.REGISTRY_CREDENTIALS.trim()).toString('base64')
      }
      return
    }
    ...
    const npm = new NpmRegistry(token, core.getInput('registry', { required: false }), npmrc_options)
    ...
    paths.forEach(file => {
      ...
      if (force === true || !content.private) {
        try {
          console.log(`Setting new version in "${file}" to ${version}`)
