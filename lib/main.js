...
async function run() {
  try {
    ...
    const token = process.env.REGISTRY_TOKEN.trim()
    const credentials = Buffer.from(process.env.REGISTRY_CREDENTIALS.trim()).toString('base64')
    ...
    const npm = new NpmRegistry(token, credentials, core.getInput('registry', { required: false }), npmrc_options)
